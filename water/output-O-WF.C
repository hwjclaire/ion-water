...
// output distance between oxygen and MLWF
...
int main (int argc, char *argv[])
{
  ...
  {
    cout << "USAGE:\n";
    cout << "output-O-WF.x [input xyz] [input mlwf]  npick  #frame nMO nskip1 nskip2 nwater\n";
    return 1;
  }
  ...
  int nwater = atoi(argv[8]), nskip2 = atoi(argv[7]), npick = atoi(argv[3]);
  assert ( nskip1 > 0 );
  assert ( nskip2 > 0 );
  assert ( npick > 0 || npick <= 4 );
  double volume;
  Cell c;
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( iframe % nskip1 == 0 ) 
      for ( int iwater = 0; iwater < waterset.size(); iwater ++ )
        polarmlwf [ iwater ] << length( waterset[iwater].owf(npick) ) << endl;
