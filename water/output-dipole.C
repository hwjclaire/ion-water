...
int main (int argc, char *argv[])
{
  ...
  {
    cout << "USAGE:\n";
    cout << "output-dipole.x [input xyz] [input mlwf] [input polariz]  #frame nMO nskip\n";
    return 1;
  }
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( nskip == 0 || iframe % nskip == 0 )
    {
      ...
      for ( int i = 0; i < waterset.size(); i ++ ) 
      {
        ...
        for ( int j = 0; j < 4 ; j ++ )
          spread.add(waterset[i].wf(j)->spread());
      }
      tm_misc.stop();
  ...
  dip.print(fdip);
  spread.print(fspread);

  cout << "Dipole: "  << dip.avg() << endl;
  cout << "Spread: "  << spread.avg() << endl;
