// output-ice192-1.x
// For a 192-D2O ice Ih surface simulation ( 8 bilayers )
// 
// Quan (Andy) Wan Fri Feb 21 14:38:18 PST 2014
...
int main (int argc, char *argv[])
{
  ...
  {
    cout << "USAGE:\n";
    cout << "output-ice192-1.x [input xyz] [input mlwf] [input quadrupole] [input polar] #frame nMO nskip\n";
    return 1;
  }
  ...
  ofstream fup,fbo,fup2,fbo2,fup3,fbo3,fup4,fbo4,fmid,fdipole;
  fup.open("surface1");
  fbo.open("surface2");
  ...
  fup3.open("ssurface1");
  fbo3.open("ssurface2");
  fup4.open("sssurface1");
  fbo4.open("sssurface2");
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( nskip == 0 || iframe % nskip == 0 )
    {
      ...
      D3vector d1(0,0,0),d2(0,0,0),d3(0,0,0),d4(0,0,0),
               d5(0,0,0),d6(0,0,0),d7(0,0,0), d8(0,0,0);
      Tensor p1,p2,p3,p4,p5,p6,p7,p8;
      ...
      for ( int iwater = 0; iwater < waterset.size(); iwater ++ )
      {
        ...
        {
          d7 += waterset[iwater].dipole();
          p7 += waterset[iwater].polariz();
          count7++;
        }
        else
        {
          d8 += waterset[iwater].dipole();
          p8 += waterset[iwater].polariz();
          count8++;
        }
      ...
      fup2 << d7;
      p7.print_inline(fup2);
      fup2 << endl;

      fup3 << d6;
      p6.print_inline(fup3);
      fup3 << endl;

      fup4 << d5;
      p6.print_inline(fup4);
      fup4 << endl;
      ...
      fdipole << d1+d2+d3+d4+d5+d6+d7+d8 << endl;
