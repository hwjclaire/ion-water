// output-ice-bulk96.x
// For a 96-D2O bulk ice simulation, including output of quadrupoles
// Quan (Andy) Wan Sat Jan 24 16:12:51 CST 2015


// optimization notes:
// should be able to further optimize for speed by
// tansposing the storage of dipole and quadrup in water sets
...
int main (int argc, char *argv[])
{
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( nskip == 0 || iframe % nskip == 0 )
    {
      ...
      D3vector  c0(12.7751,14.7086,13.9435);
      for ( int i = 0; i < waterset.size(); i ++ )
      {
        //polarmlwf[i] << D3vector();
        ...
        for ( int k = 0; k < 4; k ++ )
        {
          tt += waterset[i].wf(k)->quad()*(-2);
        }
  
        D3vector dis = waterset[i].o()->x() - c0;
        for ( int k = 0; k < 3; k ++ )
          for ( int l = 0; l < 3; l ++ )
            tt[k*3+l] += 6 * dis[k] * dis[l];

        for ( int m= 0; m < 2; m ++ )
        {
          dis = waterset[i].o()->x() + waterset[i].oh(m)- c0;
          for ( int k = 0; k < 3; k ++ )
            for ( int l = 0; l < 3; l ++ )
              tt[k*3+l] += 1 * dis[k] * dis[l];
        }

        for ( int m= 0; m < 4; m ++ )
        {
          dis = waterset[i].o()->x() + waterset[i].owf(m)- c0;
          for ( int k = 0; k < 3; k ++ )
            for ( int l = 0; l < 3; l ++ )
              tt[k*3+l] -= 2 * dis[k] * dis[l];
        }

        tt=waterset[i].quad();
        //tt.traceless();
        tt.print_inline(polarmlwf[i]);
