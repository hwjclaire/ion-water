// output-silica.x
// For a slab or bulk silica simulation
...
int main (int argc, char *argv[])
{
  ...
  {
    cout << "USAGE:\n";
    cout << "output-silica.x [input xyz] [input mlwf] [input quadrupole] [input polar] #frame\n";
    return 1;
  }
  ...
  int nframe = atoi(argv[5]), nmo=0, natom;
  double volume;
  Cell c;
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( iframe == 0 ) d0 = dipole;

    //cout << dipole << "  " << d0 << "  " << c.a()[0] <<  "  " << c.a()[4] << "  " << c.a()[8] << endl;

    for ( int idir = 0; idir < 3; idir ++ )
    {
      while ( dipole[idir] - d0[idir] > 10 )
        dipole[idir] -= c.a()[idir*4] * 2;
      while ( dipole[idir] - d0[idir] < -10 )
        dipole[idir] += c.a()[idir*4] * 2;
    }
