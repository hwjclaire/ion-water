...
class Ewald
{
  ...
  std::vector < double* > ionpolar_; // polarization of ions
  ...
  std::vector < std::vector < double > > & ionfield_tot_; // self potential of ions

  public:
  
  Ewald(Cell& cell, int nk, double kappa);

  Ewald ( Cell& cell, int nion, std::vector<D3vector*> pos, std::vector<double*> polar,
          std::vector<std::vector<double> >& elocal, int nk, double kappa );
  ...
  void self(void); // self term

  void field(void); // total field
