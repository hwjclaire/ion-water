...
int main (int argc, char *argv[])
{
  ...
  {
    cout << "USAGE:\n";
    cout << "sulfuric-quad-1st.x [input xyz] [input mlwf] [input quad] #frame nMO nskip cutoff\n";
    return 1;
  }
  ...
  fdi.open("dip-ion");
  fqi.open("quad-ion");
  fdw.open("dip-water-1st");
  fqw.open("quad-water-1st");
  fdh.open("dip-h3o");
  fqh.open("quad-h3o");
  ...
  for ( int iframe = 0; iframe < nframe; iframe ++)
  {
    ...
    if ( iframe % nskip == 0 )
    {
      ...
      for ( vector<Water>::iterator i = waterset.begin(); i < waterset.end(); i ++ )
      {
        ...
        //cout << i -> distance ( sulfate.s() ) << "  " <<  cutoff << endl;
        if ( i -> distance ( sulfate.s() ) > cutoff ) continue;
