...
// [step]
//  [water1]
//    [ BP1 ] 0 //0 for no Hbond
//    [ BP2 ] [ acceptor mlwf]
//  [water2]
//    [ BP1 ] [ acceptor mlwf]
//    [ BP1 ] 0
//    [number of water in the first shell of water1] [water1] [water2] [water3] ....
//  [water3]
//    [ BP1 ] [ acceptor mlwf1]
//    [ BP2 ] [ acceptor mlwf2]
//    [number of water in the first shell of water1] [water1] [water2] [water3] ....
...
int main (int argc, char *argv[])
{
  ...
  fhbstat.open("hbond.dat");  // output stat about first shell
    ...
    if ( nskip == 0 || iframe % nskip == 0 )
    {
      ...
      for ( int iwater = 0; iwater < waterset.size(); iwater ++ )
      {
        ...
        for ( int jwater = 0; jwater < waterset.size(); jwater ++ )
        {
          ...
          if ( d_oo < thresh1 )
          {
            ...
            for ( int ioh = 0; ioh < 2; ioh ++ )
            {
              ...
              {
                //find the closest LP WF.
                double dist[2];
                for ( int ilp = 0; ilp < 2; ilp ++ )
                {
                  D3vector bplp = waterset[iwater]. wf(ioh) -> x() 
                                - waterset[jwater]. wf(ilp+2) -> x();
                  c.images(bplp);
                  dist[ilp] = length(bplp);
                }

                //record the number of hbond bp
                if ( dist[0]> dist[1] )
                  hb[ioh].push_back(jwater*4+4);
                else
                  hb[ioh].push_back(jwater*4+3);
        ...
        for ( int ioh = 0; ioh < 2; ioh ++ )
        {
          ...
          for ( int ibp = 0; ibp < hb[ioh].size(); ibp ++ )
            hbond << hb[ioh][ibp] << "   ";
        
          hbond << endl;
        }// for ioh

        //output first solvation shell
        hbond << "    " << firstshell.size() << "  ";
        for ( int ifs = 0; ifs < firstshell.size(); ifs ++ )
          hbond << firstshell[ifs] << "   ";
        hbond << endl;

        tm_output.stop();
  
        hbstat.add(firstshell.size());
